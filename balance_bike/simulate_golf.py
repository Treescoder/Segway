...
class UpdateSimThread(QThread):
    ...
    def __init__(self, model: mujoco.MjModel, data: mujoco.MjData, parent=None) -> None:
        ...
        self.l_dof_id = self.model.jnt_dofadr[self.l_joint_id]
        self.r_dof_id = self.model.jnt_dofadr[self.r_joint_id]
    ...
    def _print_debug_info(self):
        try:
            ...
            # 计算轮速和车身速度（根据 golf_LQR.py 的逻辑）
            l_angular_vel = self.data.qvel[self.l_dof_id]
            r_angular_vel = self.data.qvel[self.r_dof_id]
            l_vel = l_angular_vel * WHEEL_RADIUS
            r_vel = r_angular_vel * WHEEL_RADIUS
            body_linear_vel = (l_vel + r_vel) / 2.0
            ...
            print(f"[t={self.data.time:.2f}s] "
                  ...
                  f"L: vel={l_vel:6.3f}m/s, torque={l_torque:6.2f}, ctrl={l_ctrl:6.2f} | "
                  f"R: vel={r_vel:6.3f}m/s, torque={r_torque:6.2f}, ctrl={r_ctrl:6.2f} | "
                  f"body_vel={body_linear_vel:6.3f}m/s | "
